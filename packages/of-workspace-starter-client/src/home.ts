...
import { getPage, getPages, deletePage, launchPage, getPageBounds } from './browser';
...
async function getResults(
    ...
    let settings = await getSettings();
    const integrationProviders: { [id: string]: Integration<unknown> } = mapIntegrationProviders(
    ...
    const onSelection = async (result: CLIDispatchedSearchResult) => {
        if (result.data !== undefined) {
            if (result.data.workspaceId !== undefined) {
                ...
                if (pageAction === HOME_ACTION_LAUNCH_PAGE || pageAction === PageTemplate.actions.launch) {
                    let pageToLaunch = await getPage(result.data.pageId);
                    await launchPage(pageToLaunch);
                } else if (pageAction === HOME_ACTION_DELETE_PAGE || pageAction === PageTemplate.actions.delete) {
                    await deletePage(result.data.pageId);
                    if (lastResponse !== undefined && lastResponse !== null) {
                        lastResponse.revoke(result.key);
                    }
                } else if (pageAction == HOME_ACTION_SHARE_PAGE || pageAction === PageTemplate.actions.share) {
                    let page = await getPage(result.data.pageId);
                    let bounds = await getPageBounds(result.data.pageId, true);
                    await share({ page, bounds });
                } else {
                    console.warn('Unknown action triggered on search result for page Id: ' + result.data.pageId);
                }
            } else {
